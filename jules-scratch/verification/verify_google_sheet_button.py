...
async def main():
    async with async_playwright() as p:
        ...
        # Use the stable `data-translate-key` attribute to locate the button,
        # bypassing any dependency on the translation script's timing.
        ...
        # Add a dummy contact directly using IndexedDB
        await page.evaluate("""() => {
        ...
        # Locate the Google Sheet button using its stable data-translate-key
        gsheet_button = page.locator('[data-translate-key="exportGSheet"]')

        await expect(gsheet_button).to_be_visible()
        await expect(gsheet_button).to_be_disabled()
